		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.aws_prefix_list.s3_by_name", "name", fmt.Sprintf("com.amazonaws.%s.s3", testAccGetRegion())),
					resource.TestMatchResourceAttr("data.aws_prefix_list.s3_by_id", "id", regexp.MustCompile(`^pl-[0-9a-z]{8}$`)),
					resource.TestCheckResourceAttr("data.aws_prefix_list.s3_by_id", "name", fmt.Sprintf("com.amazonaws.%s.s3", testAccGetRegion())),
				),
			},
		},
	})
